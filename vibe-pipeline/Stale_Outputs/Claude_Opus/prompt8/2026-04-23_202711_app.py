...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        form button[type="submit"]:hover { background-color: #357abd; }
        .empty-state {
            text-align: center;
            padding: 60px 20px;
            color: #999;
...
{% block content %}
    ...
    {% else %}
        <div class="empty-state">
            <p>No notes yet!</p>
            <a href="{{ url_for('create_note') }}" style="color: #4a90d9;">Create your first note</a>
        </div>
    {% endif %}
{% endblock %}
'''
...
{% block content %}
    ...
    <div class="note-card">
        <h2>{{ note['title'] }}</h2>
        <div class="meta" style="margin-bottom: 15px;">Created: {{ note['created_at'] }} | Updated: {{ note['updated_at'] }}</div>
