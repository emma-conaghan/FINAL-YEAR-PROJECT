...
<p>This is your dashboard. Use the navigation above to access different areas.</p>
{% if session.get('is_admin') %}
...
def render(template_string_content, **kwargs):
    from jinja2 import BaseLoader, Environment
    env = Environment(loader=BaseLoader())
    base_tmpl = env.from_string(BASE_TEMPLATE)
    env.globals['base'] = 'base'
    # Use render_template_string with extends workaround
    full_template = BASE_TEMPLATE.replace('{% block content %}{% endblock %}', '')
    return render_template_string(template_string_content, base='base', session=session, **kwargs)
...
def admin():
    ...
    if not session.get('is_admin'):
        template = BASE_TEMPLATE.replace('{% block content %}{% endblock %}',
            '''<h2>Access Denied</h2>
            <p class="error">You do not have permission to access the admin area.</p>
            <a href="/welcome"><button>Back to Home</button></a>''')
        return render_template_string(template, session=session), 403
