    ...
    if request.method == 'POST':
        ...
        if not username or not password:
            ...
            try:
                ...
                conn.close()
    return render_template_string(REGISTER_PAGE, error=error, success=success, session=session)
...
def admin():
    ...
    conn.close()
    return render_template_string(ADMIN_PAGE, users=users, session=session)
