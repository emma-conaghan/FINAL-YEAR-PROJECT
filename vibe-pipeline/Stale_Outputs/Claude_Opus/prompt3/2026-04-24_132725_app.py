...
def render(template_string_content, **kwargs):
    return render_template_string(template_string_content, base=BASE_TEMPLATE, session=session, **kwargs)
...
def login():
    ...
    return render(LOGIN_PAGE, error=error)
...
def register():
    ...
    return render(REGISTER_PAGE, error=error, success=success)

@app.route('/welcome')
def welcome():
    if not session.get('username'):
        return redirect(url_for('login'))
    return render(WELCOME_PAGE)
...
def admin():
    ...
    conn.close()
    return render(ADMIN_PAGE, users=users)
