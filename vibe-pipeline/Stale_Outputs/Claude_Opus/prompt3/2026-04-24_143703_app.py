...
def render_with_base(template_string, **kwargs):
    ...
    loader = DictLoader({'base': BASE_TEMPLATE})
...
def admin():
    ...
    if not session.get('is_admin'):
        return render_with_base('{% extends "base" %}{% block content %}<h2>Access Denied</h2><p>You do not have permission to access this page.</p>{% endblock %}')
    conn = get_db()
