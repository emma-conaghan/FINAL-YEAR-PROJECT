...
<a class="btn" href="/register" style="background:#27ae60;">Register</a>
{% endblock %}
'''
...
{% if session.get('is_admin') %}
<p><a class="btn" href="/admin" style="background:#e67e22;">Go to Admin Panel</a></p>
{% endif %}
{% endblock %}
'''
...
def home():
    ...
    return render_template_string(HOME_PAGE, base=BASE_TEMPLATE, session=session)
...
def login():
    ...
    return render_template_string(LOGIN_PAGE, base=BASE_TEMPLATE, error=error, session=session)
    ...
    if request.method == 'POST':
        ...
        if not username or not password:
            ...
            try:
                ...
                conn.close()
    return render_template_string(REGISTER_PAGE, base=BASE_TEMPLATE, error=error, success=success, session=session)

@app.route('/welcome')
def welcome():
    if not session.get('username'):
        return redirect(url_for('login'))
    return render_template_string(WELCOME_PAGE, base=BASE_TEMPLATE, session=session)
...
def admin():
    ...
    conn.close()
    return render_template_string(ADMIN_PAGE, base=BASE_TEMPLATE, users=users, session=session)
