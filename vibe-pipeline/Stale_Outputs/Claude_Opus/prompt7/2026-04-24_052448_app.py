...
<form method="GET" action="/view" class="search-form">
    <div style="flex:1">
        ...
        <input type="number" id="account_id" name="account_id" min="1" value="{{ account_id or '' }}" required>
    </div>
...
<a href="/edit/{{ user.id }}" class="btn btn-blue" style="margin-top:20px">Edit Profile</a>
{% endif %}
{% endblock %}
'''
...
<form method="POST" action="/edit/{{ user.id }}">
    ...
    <a href="/view?account_id={{ user.id }}" class="btn btn-blue">Cancel</a>
</form>
{% endblock %}
'''
...
def create():
    ...
    return render(CREATE_TEMPLATE, title='Create Account', message=message, message_type=message_type)
...
def view():
    ...
    return render(VIEW_TEMPLATE, title='View Profile', user=user, account_id=account_id, message=message, message_type=message_type)
...
def edit(user_id):
    ...
    return render(EDIT_TEMPLATE, title='Edit Profile', user=user, message=message, message_type=message_type)
