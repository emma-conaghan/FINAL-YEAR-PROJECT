...
<head>
    ...
    <style>
        ...
        .profile-field { margin-bottom: 15px; padding: 10px; background: #f8f9fa; border-radius: 4px; }
        .profile-field strong { color: #333; }
        .profile-field span { color: #666; }
...
<form method="GET" action="/view" class="search-form">
    <div style="flex: 1;">
        ...
        <input type="number" id="account_id" name="account_id" placeholder="Enter account ID" value="{{ account_id or '' }}" min="1">
    </div>
...
{% if user %}
<div style="margin-top: 30px;">
    <h2>Profile Details (ID: {{ user['id'] }})</h2>
    ...
    <div class="profile-field">
        <strong>Phone:</strong> <span>{{ user['phone'] or 'Not provided' }}</span>
    </div>
    <div class="profile-field">
        <strong>Address:</strong> <span>{{ user['address'] or 'Not provided' }}</span>
    </div>
...
<form method="POST" action="/edit/{{ user['id'] }}">
    ...
    <button type="submit" class="btn-success" style="border:none; padding:12px 24px; color:white; border-radius:4px; cursor:pointer; font-size:16px;">Update Profile</button>
    <a href="/view?account_id={{ user['id'] }}" class="btn" style="margin-left: 10px;">Cancel</a>
</form>
{% endblock %}
'''
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <a href="/view?account_id={{ user['id'] }}" class="view-link">View</a> |
                <a href="/edit/{{ user['id'] }}" class="view-link">Edit</a>
            </td>
        </tr>
        {% endfor %}
    </tbody>
...
@app.route('/')
def home():
    return render_template_string(HOME_TEMPLATE, base=BASE_TEMPLATE, title="Home - User Profile Manager")
...
def create():
    ...
    return render_template_string(CREATE_TEMPLATE, base=BASE_TEMPLATE, title="Create Account", message=message, error=error)
...
def view():
    ...
    return render_template_string(VIEW_TEMPLATE, base=BASE_TEMPLATE, title="View Profile", user=user, error=error, account_id=account_id)
