...
<head>
    ...
    <style>
        ...
        .profile-field { margin-bottom: 15px; padding: 10px; background: #f9f9f9; border-radius: 4px; }
...
<form method="POST" action="/edit/{{ user.id }}">
    ...
    <a href="/view?id={{ user.id }}" class="btn" style="margin-left: 10px;">Cancel</a>
</form>
{% endblock %}
'''
...
def render(template_str, **kwargs):
    ...
    combined = BASE_TEMPLATE.replace('{% block content %}{% endblock %}', template_str.split('{% block content %}')[1].split('{% endblock %}')[0])
    
    title = kwargs.pop('title', 'User Profile Management')
    return render_template_string(combined, title=title, **kwargs)


@app.route('/')
def home():
    return render(HOME_TEMPLATE, title='Home - User Profile Management')
...
def create():
    ...
    return render(CREATE_TEMPLATE, title='Create Account', message=message, error=error)
...
def view():
    ...
    return render(VIEW_TEMPLATE, title='View Profile', user=user, error=error, search_id=search_id)
...
def edit(user_id):
    ...
    return render(EDIT_TEMPLATE, title=f'Edit Profile - {user["name"]}', user=user, message=message, error=error)
