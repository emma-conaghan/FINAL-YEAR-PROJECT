...
{% extends "base" %}
{% block content %}
<h1>Update Profile (ID: {{ user.id }})</h1>
{% if message %}
<div class="message message-success">{{ message }}</div>
{% endif %}
{% if error %}
...
@app.route('/')
def home():
    return render(HOME_TEMPLATE, title='Home - User Profile Manager')
...
def view():
    ...
    return render(VIEW_TEMPLATE, title='View Profile', user=user, error=error, account_id=account_id)
...
def edit(user_id):
    ...
    return render(EDIT_TEMPLATE, title='Edit Profile', user=user, message=message, error=error)
